...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!item.exists) return json({ ok: false, error: "Item not found" }, { status: 404 });
    const it = item.data() as { name: string; stock: number; unitPrice: number };
    if ((it.stock || 0) < parsed.qty) return json({ ok: false, error: "Not enough stock" }, { status: 400 });
    ...
    await batch.commit();
    return json({ ok: true, id: ref.id, amount });
  } catch (error) {
