...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!book.exists) return NextResponse.json({ ok: false, error: "Book not found" }, { status: 404 });
    if (((book.data()?.available as number) || 0) <= 0) return NextResponse.json({ ok: false, error: "No copies available" }, { status: 400 });
    ...
    await batch.commit();
    return NextResponse.json({ ok: true, id: issueRef.id });
  } catch (error) {
