...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!issue.exists) return json({ ok: false, error: "Issue not found" }, { status: 404 });
    if (issue.data()?.status === "returned") return json({ ok: false, error: "Already returned" }, { status: 400 });
