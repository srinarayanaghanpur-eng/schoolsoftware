...
export async function PATCH(req: Request, { params }: RouteContext) {
  ...
  try {
    ...
    if (!source) return json({ ok: false, error: "Unknown request type" }, { status: 400 });
    ...
    if (action === "archive" || action === "restore") {
      ...
      return json({ ok: true, message: archived ? "Request archived." : "Request restored." });
    }

    if (action === "reject") {
      if (!source.allowReject) {
        return json({ ok: false, error: "This request type cannot be rejected." }, { status: 400 });
      }
      if (source.type === "leave") {
        const result = await reviewLeaveRequest(db, id, "rejected", note, token.uid);
        if (!result.ok) return json({ ok: false, error: result.error }, { status: 404 });
      } else {
        await ref.set(
...
export async function DELETE(req: Request, { params }: RouteContext) {
  ...
  if (!admin) return json({ ok: false, error: "Admin access required" }, { status: 403 });
  ...
  try {
    ...
    if (hard) {
      if (source.auditProtected) {
        return json({ ok: false, error: "Audit logs cannot be permanently deleted." }, { status: 403 });
      }
      const superAdmin = await requireSuperAdmin(req);
      if (!superAdmin) {
        return json({ ok: false, error: "Only a super admin can permanently delete." }, { status: 403 });
      }
      await ref.delete();
      return json({ ok: true, message: "Request permanently deleted." });
    }
    ...
    if (source.type === "leave") {
      ...
      if (st === "pending") {
        return json({ ok: false, error: "Approve or reject the leave request before deleting it." }, { status: 409 });
      }
    }

    await ref.set({ deletedAt: now, deletedBy: admin.uid, updatedAt: now, updatedBy: admin.uid }, { merge: true });
    return json({ ok: true, message: "Request removed." });
  } catch (error) {
