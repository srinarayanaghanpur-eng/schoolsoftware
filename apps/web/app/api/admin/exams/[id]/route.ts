...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  ...
  if (!snap.exists) return NextResponse.json({ ok: false, error: "Exam not found" }, { status: 404 });
  return NextResponse.json({ ok: true, exam: serializeDoc(snap as FirebaseFirestore.QueryDocumentSnapshot) });
}
