...
export async function GET(req: Request) {
  ...
  return json({ ok: true, vendors: snap.docs.map((d) => serializeDoc(d)) });
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    return json({ ok: false, error: error instanceof Error ? error.message : "Unable to create vendor" }, { status: 400 });
  }
}
