...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!accSnap.exists) return json({ ok: false, error: "Account not found" }, { status: 404 });
    ...
    if (txn.type === "deposit") {
      ...
      if (!txn.toAccountId) return json({ ok: false, error: "toAccountId required for transfer" }, { status: 400 });
      const toRef = db.collection("bank_accounts").doc(txn.toAccountId);
      const toSnap = await toRef.get();
      if (!toSnap.exists) return json({ ok: false, error: "Destination account not found" }, { status: 404 });
      batch.update(accRef, { currentBalance: bal - txn.amount, updatedAt: now });
    ...
    await batch.commit();
    return json({ ok: true, id: txnRef.id });
  } catch (error) {
    return json({ ok: false, error: error instanceof Error ? error.message : "Unable to record transaction" }, { status: 400 });
  }
}
