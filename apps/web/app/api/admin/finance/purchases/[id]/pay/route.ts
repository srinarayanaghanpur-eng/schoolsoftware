...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!snap.exists) return json({ ok: false, error: "Purchase not found" }, { status: 404 });

    const p = snap.data() as { amount: number; amountPaid: number; vendorName?: string };
    const newPaid = (p.amountPaid || 0) + amount;
    if (newPaid > p.amount) return json({ ok: false, error: "Payment exceeds bill amount" }, { status: 400 });
    ...
    return json({ ok: true, amountPaid: newPaid, status });
  } catch (error) {
    return json({ ok: false, error: error instanceof Error ? error.message : "Unable to record payment" }, { status: 400 });
  }
}
