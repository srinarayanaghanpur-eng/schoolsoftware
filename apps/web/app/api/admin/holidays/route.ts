...
export async function GET(req: Request) {
  try {
    ...
    return json({ ok: true, holidays: snapshot.docs.map((doc) => serializeDoc<Holiday>(doc)) });
  } catch (error) {
...
export async function POST(req: Request) {
  try {
    ...
    return json({ ok: true, holiday: { id: docRef.id, title, date, type }, message: "Holiday saved." });
  } catch (error) {
