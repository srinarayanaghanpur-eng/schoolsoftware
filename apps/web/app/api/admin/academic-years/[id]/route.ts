...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  if (!token) return NextResponse.json({ ok: false, error: "Super admin or settings manager access required" }, { status: 403 });
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  ...
  if (!snap.exists) return NextResponse.json({ ok: false, error: "Academic year not found" }, { status: 404 });
  if (snap.data()?.isActive) {
    return NextResponse.json({ ok: false, error: "Cannot delete the active academic year. Activate another year first." }, { status: 400 });
  }
  await ref.delete();
  return NextResponse.json({ ok: true });
}
