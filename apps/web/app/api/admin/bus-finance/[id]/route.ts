...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!String(body.auditNote || "").trim()) {
      return json({ ok: false, error: "An audit note is required for financial changes" }, { status: 400 });
    }
    const existing = snap.data() as Record<string, unknown>;
    if (body.status === "active" && ["completed", "closed"].includes(String(existing.status)) && token.role !== "admin" && token.role !== "super_admin") {
      return json({ ok: false, error: "Only an administrator can reopen a loan" }, { status: 403 });
    }
    ...
    const auditRef = adminDb().collection("bus_finance_audit_logs").doc();
    const batch = adminDb().batch();
    batch.update(ref, update);
    batch.set(auditRef, {
      busFinanceId: params.id,
      action: body.status === "closed" ? "loan_closed" : body.status === "active" ? "loan_reopened_or_updated" : "loan_updated",
      before: existing,
      changes: Object.fromEntries(Object.entries(update).filter(([key]) => key !== "updatedAt")),
      reason: String(body.auditNote).trim(),
      createdBy: token.uid,
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!String(body.auditReason || "").trim()) {
      return json({ ok: false, error: "Admin confirmation requires an audit reason" }, { status: 400 });
    }
    const db = adminDb();
    ...
    await adminDb().collection("bus_finance_audit_logs").add({
      busFinanceId: params.id,
      action: "loan_deleted",
      snapshot: snap.data(),
      reason: String(body.auditReason).trim(),
      createdBy: token.uid,
      createdAt: FieldValue.serverTimestamp(),
    });
    await ref.delete();
