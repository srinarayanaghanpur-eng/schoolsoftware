...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const record: Record<string, unknown> = {
      ...
      totalAmountPaid: 0,
      outstandingPrincipal: Number(body.totalLoanAmount) || 0,
      nextEmiDueDate: String(body.loanStartDate),
      interestRate: body.interestRate !== undefined && body.interestRate !== "" ? Number(body.interestRate) : null,
      status: "active",
      notes: body.notes ? String(body.notes) : "",
      documentUrls: Array.isArray(body.documentUrls) ? body.documentUrls.map(String) : [],
      createdBy: token.uid,
      createdAt: now,
      updatedAt: now,
    };
