...
import { maintenanceLogCreateSchema } from "@sri-narayana/shared";
...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ ok: true, maintenanceLogs: logs, vehicles });
}
...
export async function POST(req: Request) {
  ...
  if (!token) return NextResponse.json({ ok: false, error: "Access denied" }, { status: 403 });
  try {
    const parsed = maintenanceLogCreateSchema.parse(await req.json());
