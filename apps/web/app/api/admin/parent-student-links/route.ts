...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ ok: false, error: "Provide parentUid or studentId query param" }, { status: 400 });
  } catch (error) {
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ ok: true, id, message: "Parent linked to student." });
  } catch (error) {
