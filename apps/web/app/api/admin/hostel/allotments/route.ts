...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ ok: true, allotments: snap.docs.map((d) => serializeDoc(d)) });
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!room.exists) return NextResponse.json({ ok: false, error: "Room not found" }, { status: 404 });
    const r = room.data() as { number: string; capacity: number; occupied: number };
    if ((r.occupied || 0) >= r.capacity) return NextResponse.json({ ok: false, error: "Room is full" }, { status: 400 });
