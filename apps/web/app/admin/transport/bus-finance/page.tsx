...
const STATUS_STYLE: Record<string, string> = {
  ...
  emi_due: "bg-[#fff8ea] text-[#c27803]",
  paid_this_month: "bg-[#e7f6ec] text-[#1f8a4c]",
  overdue: "bg-[#ffebed] text-[#d84d5b]",
  completed: "bg-[#e7f6ec] text-[#1f8a4c]",
  closed: "bg-[#f1f2f6] text-[#6b7280]",
};

interface OverdueRow { overdueAmount?: number }
interface MonthlyRow { emiAmount?: number; paidAmount?: number; status?: string }
...
export default function BusFinancePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setPaidThisMonth((monthly.rows ?? []).reduce((s, r) => s + (Number(r.paidAmount) || 0), 0));
    } catch (err) {
  ...
  const cards = useMemo(() => {
    ...
    const nextDue = records
      ...
      .map((r) => r.nextEmiDueDate || r.loanEndDate)
      .sort()[0];
  ...
  const closeLoan = async (id: string) => {
    ...
    try {
      await adminApiRequest(`/api/admin/bus-finance/${id}`, {
        method: "PATCH",
        body: JSON.stringify({ status: "closed", auditNote: "Loan closed from Bus EMI dashboard" }),
      });
  ...
  const remove = async (id: string, vehicleNumber: string) => {
    if (!canDelete) return;
    const auditReason = window.prompt(`Delete finance record for ${vehicleNumber}? Enter the mandatory audit reason:`);
    if (!auditReason?.trim()) return;
    setBusyId(id);
    setError("");
    setSuccess("");
    try {
      await adminApiRequest(`/api/admin/bus-finance/${id}`, { method: "DELETE", body: JSON.stringify({ auditReason }) });
  ...
  if (!canView) {
    return (
      <>
        <PageHeader title="Bus EMI Management" description="Vehicle loan and EMI management." />
  ...
  return (
    <>
      <PageHeader
        title="Bus EMI Management"
        description="Manage monthly EMI repayments for buses and vehicles bought on finance."
        action={
          canCreate ? (
            <Link href="/admin/finance/bus-emi/create" className="btn-primary">
      ...
      <section className="space-y-5 p-4 md:p-7">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
          <SummaryCard icon={<Bus size={18} />} label="Total Bus Loans" value={String(records.length)} />
          <SummaryCard icon={<IndianRupee size={18} />} label="Monthly Commitment" value={inr(records.filter((r) => !["completed", "closed"].includes(r.status)).reduce((s, r) => s + Number(r.emiAmount || 0), 0))} />
          <SummaryCard icon={<IndianRupee size={18} />} label="Paid This Month" value={inr(paidThisMonth)} />
          <SummaryCard icon={<CalendarClock size={18} />} label="Pending This Month" value={inr(currentMonthDue)} />
          <SummaryCard icon={<AlertTriangle size={18} />} label="Overdue Amount" value={inr(overdueAmount)} highlight={overdueAmount > 0} />
          <SummaryCard icon={<IndianRupee size={18} />} label="Outstanding Balance" value={inr(records.reduce((s, r) => s + Number(r.outstandingPrincipal ?? r.totalLoanAmount), 0))} />
        </div>
        ...
        <div className="card overflow-hidden p-0">
          <div className="flex items-center justify-between gap-3 border-b border-[#edf0f7] px-5 py-3">
            ...
            <Link href="/admin/finance/bus-emi/reports" className="text-sm font-semibold text-[#3033a1] hover:underline">
              View Reports →
            </Link>
          </div>
                    ...
                    <tr key={r.id} className="border-t border-[#edf0f7]">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            href={`/admin/finance/bus-emi/${r.id}`}
