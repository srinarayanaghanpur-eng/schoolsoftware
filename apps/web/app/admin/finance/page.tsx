...
export default function FinanceDashboardPage() {
  ...
  const [busLoans, setBusLoans] = useState<BusFinance[]>([]);
  ...
  useEffect(() => {
    if (!selectedYear?.id || !hasPermission(role, "bus_finance.view")) {
      setBusLoans([]);
      return;
    }
    const query = new URLSearchParams({ academicYearId: selectedYear.id, pageSize: "100" });
    void adminApiRequest<{ records: BusFinance[] }>(`/api/admin/bus-finance?${query}`)
      .then((result) => setBusLoans(result.records ?? []))
      .catch(() => setBusLoans([]));
  }, [role, selectedYear?.id]);
  ...
  const quickActions = [
    ...
    ...(hasPermission(role, "bus_finance.view") ? [{ label: "Bus EMI", icon: <Banknote size={17} />, onClick: () => { window.location.href = "/admin/finance/bus-emi"; } }] : []),
  ];
  ...
  return (
    ...
    >
      ...
      {hasPermission(role, "bus_finance.view") && (
        <Link href="/admin/finance/bus-emi" className="block rounded-2xl border border-[#dbeafe] bg-gradient-to-r from-[#eff6ff] to-white p-4 shadow-sm transition hover:border-[#93c5fd]">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-wide text-[#2563eb]">Pending Financial Obligations</p>
              <h3 className="mt-1 text-base font-extrabold text-[#1e293b]">Bus EMI</h3>
              <p className="mt-1 text-xs font-medium text-[#64748b]">{busLoans.length} loan{busLoans.length === 1 ? "" : "s"} tracked · Open management and payment history</p>
            </div>
            <div className="grid grid-cols-2 gap-x-5 gap-y-1 text-right">
              <div><p className="text-[10px] font-bold uppercase text-[#94a3b8]">Monthly</p><p className="font-extrabold text-[#1e293b]">{formatINR(busLoans.filter((loan) => !["completed", "closed"].includes(loan.status)).reduce((sum, loan) => sum + Number(loan.emiAmount || 0), 0))}</p></div>
              <div><p className="text-[10px] font-bold uppercase text-[#94a3b8]">Outstanding</p><p className="font-extrabold text-[#dc2626]">{formatINR(busLoans.reduce((sum, loan) => sum + Number(loan.outstandingPrincipal ?? loan.totalLoanAmount), 0))}</p></div>
            </div>
          </div>
        </Link>
      )}
