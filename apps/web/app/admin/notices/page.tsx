...
type Notice = { id: string; title: string; body: string; audienceRoles: string[]; channels: string[]; createdAt?: string };
...
export default function NoticesPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      await adminApiRequest("/api/admin/notices", { method: "POST", body: JSON.stringify({ title, body, audienceRoles: [...audRoles], channels: [...channels] }) });
      setTitle(""); setBody(""); setAudRoles(new Set()); setChannels(new Set(["app"])); setShow(false); await load();
      ...
      <section className="space-y-4 p-4 md:p-7">
        ...
        {show && (
          <form onSubmit={submit} className="card space-y-4 p-5">
            ...
            <div>
              <p className="mb-2 text-sm font-semibold text-[#303247]">Audience (empty = everyone)</p>
