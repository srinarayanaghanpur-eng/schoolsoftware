    ...
    <section className="p-4 md:p-7">
      <div className="card flex max-w-2xl items-start gap-4 p-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm font-medium text-[#7d86a8]">Your role cannot view academic years.</p>
        </div>
      </div>
    </section>
  );
...
export default function AcademicYearsPage() {
  ...
  if (accessDenied || !canView) {
    return (
      <>
        <PageHeader title="Academic Years" description="Manage school year windows and the active operating year." />
        <AccessNotice />
      </>
    );
  }
  ...
  return (
    <>
      <PageHeader
        ...
        description={activeYear ? `${activeYear.name} is active for current operations.` : years.length ? "No academic year is active." : "Create the first academic year to begin."}
        action={
          canWrite ? (
