...
export async function generateEmiSchedule(financeId: string, source: ScheduleSource): Promise<number> {
  ...
  let created = 0;
  let openingBalance = Math.max(0, Number(source.totalLoanAmount) || (Number(source.emiAmount) * source.totalEmis));
  const monthlyRate = Math.max(0, Number(source.interestRate) || 0) / 1200;
  ...
  for (let n = 1; n <= source.totalEmis; n++) {
    ...
    const interestComponent = Math.min(openingBalance, Math.round(openingBalance * monthlyRate * 100) / 100);
    const principalComponent = Math.min(openingBalance, Math.max(0, Math.round((source.emiAmount - interestComponent) * 100) / 100));
    const closingBalance = Math.max(0, Math.round((openingBalance - principalComponent) * 100) / 100);
    ...
    batch.set(ref, {
      ...
      closingBalance,
      paidAmount: 0,
      status: dueIso < today ? "overdue" : dueIso === today ? "due" : "upcoming",
      lateFee: 0,
      createdAt: now,
      updatedAt: now,
    });
...
export function derivePaymentStatus(paidAmount: number, emiAmount: number, dueDate: string): "upcoming" | "due" | "paid" | "partial" | "overdue" {
...
export async function recalcFinanceSummary(financeId: string): Promise<void> {
  ...
  for (const doc of paymentsSnap.docs) {
    ...
    totalAmountPaid += Number(p.paidAmount) || 0;
    if (status === "paid" || status === "partial") {
      const paid = Number(p.paidAmount) || 0;
      const scheduledPrincipal = Number(p.principalComponent) || 0;
      const scheduledInterest = Number(p.interestComponent) || 0;
      const scheduledTotal = scheduledPrincipal + scheduledInterest;
      const ratio = scheduledTotal > 0 ? Math.min(1, paid / scheduledTotal) : 0;
      principalPaid += scheduledPrincipal * ratio;
      interestPaid += scheduledInterest * ratio;
    }
    if (status === "paid") {
      paidEmis++;
      if (String(p.paymentDate || "").slice(0, 7) === today.slice(0, 7)) hasPaidThisMonth = true;
      continue;
    }
    const dueDate = String(p.dueDate || "");
    if (!nextEmiDueDate || (dueDate && dueDate < nextEmiDueDate)) nextEmiDueDate = dueDate;
  ...
  if (status !== "closed") {
    if (pendingEmis === 0 && totalEmis > 0) status = "completed";
    else if (hasOverdue) status = "overdue";
    else if (hasPaidThisMonth) status = "paid_this_month";
    else if (nextEmiDueDate && nextEmiDueDate <= isoDate(new Date(Date.now() + 7 * 86_400_000))) status = "emi_due";
    else status = "active";
  }
  const originalPrincipal = Math.max(0, Number(finance.totalLoanAmount) || 0);
  ...
  await financeRef.update({
    ...
    outstandingPrincipal: Math.max(0, Math.round((originalPrincipal - principalPaid) * 100) / 100),
    nextEmiDueDate,
