...
export async function fetchBusFinance() {
  const result = await adminGet<{ records: BusFinanceRecord[] }>("/api/admin/bus-finance?pageSize=50");
  return result.records ?? [];
}

export async function fetchBusEmiSchedule(busFinanceId: string) {
  const result = await adminGet<{ schedule: BusEmiPayment[] }>(
    `/api/admin/bus-finance/${encodeURIComponent(busFinanceId)}/emi-schedule`
  );
  return result.schedule ?? [];
}

export async function markBusEmiPaid(
  paymentId: string,
  input: { paidAmount: number; paymentMode: string }
) {
  return adminPatch<{ payment: BusEmiPayment }>(
    `/api/admin/bus-emi-payments/${encodeURIComponent(paymentId)}`,
    {
