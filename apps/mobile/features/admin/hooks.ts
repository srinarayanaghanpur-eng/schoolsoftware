...
import {
  fetchBusFinance, fetchDashboardStats, fetchFinanceSummary, fetchLeaveRequests, fetchNotices,
  fetchRecentPayments, fetchTeachers, fetchTodayAttendance,
  type AdminPayment, type AdminTeacher, type BusFinanceRecord, type DashboardStats, type FinanceSummary,
...
export function useBusFinance() {
  const { data, loading, error, refresh } = useCachedFetch<BusFinanceRecord[]>(
    "admin-bus-finance",
    fetchBusFinance,
    5
  );
  return { buses: data ?? [], loading, error, refresh };
}
